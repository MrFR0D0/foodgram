...
class Command(BaseCommand):
    """Загружает список пользователей из users.csv и создает суперпользователя."""

    help = 'Загрузка пользователей из data/users.csv и создание суперпользователя'
    ...
    def handle(self, *args, **options):
        ...
        if not User.objects.filter(username='admin').exists():
            ...
            self.stdout.write(self.style.SUCCESS(
                "Суперпользователь для администрирования - email: admin@admin.com пароль: admin"
            ))
        else:
            self.stdout.write(self.style.WARNING(
                "Суперпользователь для администрирования - email: admin@admin.com пароль: admin"
