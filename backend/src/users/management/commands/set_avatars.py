...
class Command(BaseCommand):
    """Назначает аватарки на основе пола из users.csv."""

    help = 'Назначение аватарок пользователям с сайта randomuser.me на основе CSV'
    ...
    def _get_gender_map(self):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"Файл не найден: {users_file_path}!"))
        return gender_map

    def handle(self, *args, **options):
        import requests # <-- ИМПОРТ ПЕРЕМЕЩЕН
        
        gender_map = self._get_gender_map()
        if not gender_map:
            self.stdout.write(self.style.ERROR("Не удалось получить данные о гендере пользователей из CSV!"))
            return
        ...
        for user in users_without_avatar:
            ...
            if not gender:
                self.stdout.write(self.style.WARNING(f"Гендер для пользователя {user.username} не найден в CSV! Пропускаю..."))
                continue
            ...
            try:
                ...
                #     f"Пользователю '{user.username}' назначена {gender} аватарка."
                # ))

            except requests.exceptions.RequestException as e:
                self.stdout.write(self.style.ERROR(f"Не удалось получить аватарку для {user.username}: {e}!"))
            except (KeyError, IndexError):
                self.stdout.write(self.style.ERROR(f"Не удалось разобрать ответ от API для {user.username}!"))
