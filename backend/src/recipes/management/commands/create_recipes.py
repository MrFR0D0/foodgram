...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not authors:
            self.stdout.write(self.style.ERROR('Нет пользователей для назначения авторами. Запустите `import_users`!'))
            return

        if not Ingredient.objects.exists():
            self.stdout.write(self.style.ERROR('Нет ингредиентов в базе. Запустите `update_ingredients`!'))
            return
            ...
            with open(recipes_csv_path, mode='r', encoding='utf-8') as f:
                ...
                for row in reader:
                    ...
                    try:
                        ...
                        cooking_time = int(cook_time_match.group(0)) if cook_time_match else 0
                        ...
                        if image_filename:
                            ...
                            if os.path.exists(image_path):
                                ...
                                self.stdout.write(self.style.WARNING(f"Файл изображения '{image_filename}' не найден. Рецепт создан без картинки."))
                        ...
                        ingredient_entries = re.split(r', (?=[А-ЯЁ])', ingredients_str)
                        for entry in ingredient_entries:
                            ...
                            amount = int(float(amount_match.group(1).replace(',', '.'))) if amount_match else 1

                            # Ищем ингредиент в БД
                            ingredient_obj = Ingredient.objects.filter(name__istartswith=ing_name).first()
                            if ingredient_obj:
                                ...
                                self.stdout.write(self.style.WARNING(f"Ингредиент '{ing_name}' не найден. Пропускаю."))

                        created_count += 1
                        # self.stdout.write(self.style.SUCCESS(f"Рецепт '{recipe_name}' успешно создан."))

                    except Exception as e:
                        self.stdout.write(self.style.ERROR(f"Ошибка при создании рецепта '{recipe_name}': {e}!"))

        except FileNotFoundError:
            self.stdout.write(self.style.ERROR(f"Файл не найден: {recipes_csv_path}!"))
            return

        # self.stdout.write("\n--- Статистика ---")
        # self.stdout.write(self.style.SUCCESS(f"Создано новых рецептов: {created_count}"))
